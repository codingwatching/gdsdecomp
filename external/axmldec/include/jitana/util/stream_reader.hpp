...
namespace jitana {
    ...
    /// memory space.
    ///
    /// On out-of-range access the reader sets an internal `error` flag and
    /// returns safe defaults instead of throwing. Callers should consult
    /// `has_error()` to detect failures.
    class stream_reader {
    public:
        /// Creates a stream_reader instance.
        stream_reader() noexcept
        {
            set_memory_range(nullptr, nullptr);
        }

        /// Creates a stream_reader instance.
        explicit stream_reader(const void* first, const void* last) noexcept
        {
            set_memory_range(first, last);
        }

        /// Sets the begin and end pointers.
        void set_memory_range(const void* first, const void* last) noexcept
        {
        ...
        /// Returns true if any prior operation read past the end of the buffer
        /// or otherwise failed.
        ...
        /// Resets the error flag so the reader can be reused.
        ...
        /// Moves the head.
        void move_head(size_t pos = 0) const noexcept
        {
        ...
        void move_head_forward(int off = 0) const noexcept
        {
            head_ptr_ += off;
            validate_head();
        }
        ...
        ///
        /// On out-of-range access, the error flag is set and a reference to a
        /// zero-initialized static value is returned.
        template <typename T>
        const T& peek() const noexcept
        {
            static const T zero{};
            if (!validate_head(sizeof(T))) {
                return zero;
            }
        ...
        /// Returns the reference to the value of specified type from the head.
        ///
        /// The head moves forward to point the next value as a result. On
        /// out-of-range access the error flag is set and the head is not
        /// advanced.
        template <typename T>
        ...
        uint32_t get_uleb128() const noexcept
        {
            uint32_t decoded = 0;
        ...
        ///
        /// The head moves forward to point the next value as a result.
        int32_t get_sleb128() const noexcept
        {
            uint32_t decoded = 0;
        ...
        ///
        /// The head moves forward to point the next value as a result.
        uint32_t get_uleb128p1() const noexcept
        {
        ...
        ///
        /// The head moves forward to point the next value as a result. Returns
        /// false (and sets the error flag) if the read would overflow.
        bool get_array(uint8_t* array, size_t length) const noexcept
        {
        ...
        ///
        /// The head moves forward to point past the terminator. On overflow
        /// (no terminator before end of buffer) the error flag is set and an
    ...
    private:
        /// Validates the head pointer against the begin/end pointers. Sets the
        /// error flag and returns false on overflow.
        bool validate_head(size_t type_size = 0) const noexcept
        {
        ...
        /// Sticky error flag set on out-of-range access.
